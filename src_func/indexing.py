...
# Use pandas to read files and clean
df = pd.read_csv('data/publications/final_database_of_papers.csv', index_col=0)

def remove_empty_rows(df, column):
    """" function that sepearates empty rows in a specific column in a dataframe
    and returns two dataframes one wiht non-empty rows and other with empty rows
...
#pickle the index for abstracts 
...
#pickle the embeddings for abstracts 
