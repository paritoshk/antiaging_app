...
def get_dict(df, column):
    return dict(zip(df.index, df[column]))
...
def getentities_fromabstract(abstract):
    ...
    d_entity_list = dd.from_array(entity_list)

    for s in sentences:
        d_entity_list = d_entity_list.append(dd.from_array([query_plain(s)]))
        
    entity_list = d_entity_list.compute()
    ...
    df_text_sha = dd.from_pandas(pd.DataFrame(tuple_list_main, columns=['text', 'text_sha256']), npartitions=1)
    df_sub_entity = dd.from_pandas(pd.DataFrame(tuple_list_sub, columns=['entity_id', 'other_ids', 'entity_type', 'entity']))
...
# Convert the list of delayed objects into a Dask DataFrame
ddf = dd.from_delayed(delayed_dfs)
#This will create a Dask DataFrame (ddf) with the same columns as the Pandas DataFrame returned by getentities_fromabstract.
